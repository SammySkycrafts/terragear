...
tgSurface::~tgSurface() {
    ...
    delete[] ya;
    delete[] za;
    gsl_spline2d_free(surface_spline);
...
void tgSurface::fit() {
    SG_LOG(SG_GENERAL, SG_DEBUG, "Creating bicubic spline");

    surface_spline = gsl_spline2d_alloc(gsl_interp2d_bicubic, Pts->cols(), Pts->rows());
    xa = new double[Pts->cols()];
    ya = new double[Pts->rows()];
    za = new double[Pts->cols() * Pts->rows()];
    ...
    for (int i = 0; i < Pts->cols(); i++) {
        for (int j = 0; j < Pts->rows(); j++) {
            SGGeod p = Pts->element(i, j);
            xa[i] = p.getLongitudeDeg() - area_center.getLongitudeDeg();
            ya[j] = p.getLatitudeDeg() - area_center.getLatitudeDeg();
            gsl_spline2d_set(surface_spline, za, i, j, p.getElevationM() - area_center.getElevationM());
        }
    }
    gsl_spline2d_init(surface_spline, xa, ya, za, Pts->cols(), Pts->rows());
...
double tgSurface::query( SGGeod query ) const {
    ...
    double result = gsl_spline2d_eval(surface_spline, x, y, xacc, yacc);
